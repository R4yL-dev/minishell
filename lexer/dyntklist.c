...
/*   Updated: 2023/10/30 11:52:35 by lray             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	dyntklist_add(t_dyntklist *tklist, int type, char *value)
{
	...
	tklist->array = ft_realloc(tklist->array, \
	sizeof(t_token) * actual_size, sizeof(t_token) * (actual_size + 1));
	if (tklist->array == NULL)
	{
...
int	dyntklist_delone(t_dyntklist *tklist, int id)
{
	...
	if (tklist->size > 0)
		tklist->array = ft_realloc(tklist->array, \
		sizeof(t_token) * (tklist->size + 1), sizeof(t_token) * tklist->size);
	if (tklist->array == NULL)
	{
		ft_puterror("Realloc failed");
		return (0);
